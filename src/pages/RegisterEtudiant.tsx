...
function hashPassword(password: string) {
  // Très simple, pour démo uniquement : à remplacer par une vraie solution côté backend/Edge Function !
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        // id_etudiant généré automatiquement
      });
    if (insertError) {
  ...
  return (
    <main className="flex items-center justify-center min-h-[70vh] bg-background">
      <form onSubmit={handleSubmit} className="bg-card shadow-lg rounded-lg p-8 w-full max-w-md">
        <h1 className="text-2xl font-bold mb-4 text-primary text-center">Inscription Étudiant</h1>
        {session && (
        ...
        <div className="mb-6">
          <label htmlFor="pwd" className="block font-medium mb-2">Code étudiant</label>
          <Input  type="text" placeholder="Code fourni par l'etablissement" required />
        </div>
        <Button type="submit" className="w-full mb-2">Créer un compte étudiant</Button>
